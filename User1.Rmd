...
library(rworldmap)
map.world <- map_data(map="world") %>% mutate(Cricket=region %in% c("Australia", "UK", "India", "Srilanka", "Pakistan", "Bangladesh", "New Zealand","South Africa","West Indies","Zimbabwe"))


gg <- ggplot()
gg <- gg + theme(legend.position="none")
gg <- gg + geom_map(data=map.world, map=map.world, aes(map_id=region, x=long, y=lat, fill=Cricket))+ theme_void() + theme(legend.position = "bottom")+ coord_equal() 
gg
...
Consecutive_Century_F <- odibat_F%>%arrange(Player, Date, Runs) %>%
mutate(new_col=Runs >= 100) %>%group_by(Player,Country)%>%
mutate(Run_length = rep(rle(new_col)$lengths,rle(new_col)$lengths), Runvar = rep(rle(new_col)$values,rle(new_col)$lengths))%>%filter(new_col==TRUE)%>%
summarize(Con_cent=max(Run_length))%>%filter(Con_cent>=2)%>%select(Player,Country)%>%
knitr::kable()
...
odibat_M_1%>%filter(Matches=="More than 150")%>% mutate(mdu=ave(Ducks_Rate),mhu=ave(HighScore))->Average_M_1

odibat_M_1 %>% filter(Matches=="More than 150")%>% ggplot(aes(y = HighScore, x = Ducks_Rate,col=Average,Player=Player,Country=Country)) + geom_hline(yintercept=Average_M_1$mhu)+geom_vline(xintercept=Average_M_1$mdu)+geom_point() + scale_color_distiller(palette="RdYlGn") + 
...
ggtitle("Batsmen playing more than 150 matches") + theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=22, hjust=0)) +
theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14)) +
...
Dist_dismissal %>% ggplot(aes(x=reorder(Dismissal,n),y=n,fill=Name))+geom_bar(stat = "identity")+ facet_grid(Runs~Name) + xlab("Dismissals") + ylab("Count") + scale_color_brewer(palette = "Dark2")+ ggtitle("Distribution of Dismissals :  ODI Players") +theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14, hjust=0)) +
  theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+
...
Act_players <- rbind(Act_players_odi,Act_players_test,Act_players_t20)%>%left_join(players_comp,by=c("player_id"="ID"))%>%select(Name,Runs,X4s,X6s,SR,Format)


g11<- Act_players%>%ggplot(aes(y = Runs, x = Name, col= Format)) +
  geom_boxplot()  +  xlab("Players") + ylab("Distribution of Runs")+theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14, hjust=0)) +
  theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+
...
g12<- Act_players%>%ggplot(aes(y = as.numeric(SR), x = Name, col= Format)) +
  geom_boxplot() + xlab("Players") + ylab("Distribution of Strike Rate")+  xlab("Players") + ylab("Distribution of SR")+theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14, hjust=0)) +
  theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+
  theme(axis.text = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+ theme(legend.text=element_text(family = "Trebuchet MS", color="#666666", face="bold", size=12),legend.position = "bottom")
...
```{r,X6_Formats,echo=FALSE,fig.caption="Number of 6s across different game formats", out.width="100%",out.height="100%",fig.width=14,fig.height=11}



g13<- Act_players%>%ggplot(aes(y = as.numeric(X6s), x = Name, col= Format)) +
  geom_boxplot() + xlab("Players") + ylab("Distribution of X6s")+ xlab("Players") + ylab("Distribution of X6s")+theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14, hjust=0)) +
  theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+
  theme(axis.text = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+ theme(legend.text=element_text(family = "Trebuchet MS", color="#666666", face="bold", size=12),legend.position = "bottom")
...
```{r,X4_Formats,echo=FALSE,fig.caption="Number of 4s of Players across different game formats", out.width="100%",out.height="100%",fig.width=14,fig.height=11}


g14<- Act_players%>%ggplot(aes(y = as.numeric(X4s), x = Name, col= Format)) +
  geom_boxplot() + xlab("Players") + ylab("Distribution of X4s")+  xlab("Players") + ylab("Distribution of X4s")+theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14, hjust=0)) +
  theme(axis.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+
  theme(axis.text = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=14))+ theme(legend.text=element_text(family = "Trebuchet MS", color="#666666", face="bold", size=12),legend.position = "bottom")
